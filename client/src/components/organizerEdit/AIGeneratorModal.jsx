            ...
            <div className={modalStyles.panelXl}>
                ...
                <div className={modalStyles.contentSectionScroll}>
                    <div className={panelStyles.mutedBox}>
                        <div className={components.organizer.aiPromptHeader}>
                            <div>
                            ...
                            <button onClick={handleCopyPrompt} className={cx(buttonStyles.secondary, components.organizer.aiPromptCopyBtn)}>
                                Copy Prompt
                            </button>
                        </div>
                        <pre className={components.organizer.aiPromptCode}>{promptText}</pre>
                    </div>
                    ...
                    {loading && (
                        <div className={modalStyles.stackSm}>
                            <div className={panelStyles.loadingBox}>
                                <Loader2 size={16} className={components.organizer.spin} /> Generating questions...
                            </div>
                            {[1, 2, 3].map((n) => (
                                <div key={n} className={components.organizer.aiSkeleton} />
                            ))}
                        </div>
                    )}
                    ...
                    {!loading && activeTab === 'preview' && generated.map((question, index) => (
                        <div key={`${question.text}-${index}`} className={cx(cardStyles.base, components.organizer.aiQuestionCardPad)}>
                            <div className={components.organizer.aiQuestionHeader}>
                                <p className={textStyles.bodyStrong}>{index + 1}. {question.text}</p>
                                <span className={components.organizer.aiQuestionDifficulty}>{question.difficulty || 'mixed'}</span>
                            </div>
                            <div className={components.organizer.aiOptionGrid}>
                                {question.options.map((option) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={cx(
                                                components.organizer.aiOptionBase,
                                                isCorrect ? components.organizer.aiOptionCorrect : components.organizer.aiOptionWrong,
                                            )}
                                        >
                                            {option}
                                        </div>
                            ...
                            {question.explanation && (
                                <p className={cx(components.organizer.aiExplainMargin, textStyles.captionStrong)}>
                                    <span className={components.organizer.aiExplainLabel}>Explanation:</span> {question.explanation}
                                </p>
                            )}
                        </div>
                    ))}
                    ...
                    {!loading && activeTab === 'json' && generated.length > 0 && (
                        <div className={modalStyles.stackMd}>
                            <button onClick={handleCopyJSON} className={cx(buttonStyles.secondary, components.organizer.aiPromptCopyBtn)}>Copy JSON</button>
                ...
                <div className={modalStyles.footerRight}>
                    <button
                        ...
                        className={cx(buttonStyles.primary, components.organizer.aiGenerateBtn)}
                    >
                        {loading ? <Loader2 size={14} className={components.organizer.spin} /> : <Sparkles size={14} />}
                        Generate
                    </button>
                    <button
                        type="button"
                        ...
                        className={cx(buttonStyles.secondary, components.organizer.aiActionBtn, components.organizer.aiPromptCopyBtn)}
                    >
                    ...
                    <button
                        ...
                        className={cx(buttonStyles.success, components.organizer.aiGenerateBtn)}
                    >
                        {saving ? <Loader2 size={14} className={components.organizer.spin} /> : <Save size={14} />}
                        Save To Quiz
                    </button>
                </div>
            </div>
