...
const PricingCard = ({ plan }) => (
    ...
    >
        ...
        {/* 🔥 CTA ALWAYS BOTTOM */}
        <Link
            to={plan.isComingSoon ? '#' : (plan.name === 'Free' ? '/register' : `/upgrade/${plan.name.toLowerCase()}`)}
            className={`${plan.featured
                ? 'btn-premium bg-gradient-to-r from-indigo-500 to-indigo-600 shadow-[0_10px_40px_rgba(99,102,241,0.4)]'
                : plan.isComingSoon
                    ? 'theme-surface-soft border border-dashed theme-border opacity-50 cursor-not-allowed'
                    : 'theme-surface-soft border theme-border hover:border-indigo-500/40'
