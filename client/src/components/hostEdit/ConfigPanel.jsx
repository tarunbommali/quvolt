                ...
                <div className={components.host.configField}>
                    ...
                    >
                        ...
                        {(() => {
                            ...
                            const isMCQ = activeQuestion?.questionType === 'multiple-choice' || !activeQuestion?.questionType;
                            const displayOptions = isMCQ 
                                ? [...options, ...Array(Math.max(0, 4 - options.length)).fill('')].slice(0, 4)
                                : options;

                            return displayOptions.map((option, index) => (
                                <option key={index} value={index}>
                                    Option {index + 1}: {option ? (option.length > 20 ? `${option.substring(0, 20)}...` : option) : 'Empty'}
                                </option>
                            ));
                        })()}
                    </select>
