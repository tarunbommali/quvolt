...
import { Play, Zap, Clock, Copy, Check } from 'lucide-react';
...
const InviteRoom = () => {
    ...
    useEffect(() => {
        ...
        const tid = setInterval(async () => {
            try {
                const response = await api.get(`/quiz/session/${code.toUpperCase()}/state`);
                if (response.data.success && response.data.data.participants) {
                    useQuizStore.getState().setParticipants(response.data.data.participants);
                }
            } catch (err) {
