...
const TemplateCard = ({ template, view = 'grid', cloning, onEdit, onDelete, onClone, onGoLive, onPrefetch, onSessionSettings, onViewHistory }) => {
    ...
    const meta = useMemo(() => [
        { label: 'Created', value: toDate(template.createdAt), icon: CalendarDays },
        { label: 'Last Activity', value: toRelativeTime(template.updatedAt), icon: Clock3 },
        { label: 'Slides', value: `${template.questions?.length || 0}`, icon: Zap },
        { label: 'Sessions Live', value: `${template.sessionCount ?? 0}`, icon: Radio },
    ], [template.createdAt, template.questions?.length, template.sessionCount, template.updatedAt]);
                ...
                >
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-6">
                        <div className="flex min-w-62.5 flex-col gap-1 md:w-110 md:shrink-0">
                            <h3 className="truncate text-sm font-semibold theme-text-primary md:text-base" title={template.title}>
                                {template.title}
                            </h3>
                            <div className="flex flex-wrap gap-4 text-xs theme-text-muted">
                                <span className="inline-flex items-center gap-1.5">
                                    <CalendarDays size={13} className="text-gray-400" />
                                    Created {toDate(template.createdAt)}
                                </span>
                                <span className="inline-flex items-center gap-1.5">
                                    <Clock3 size={13} className="text-gray-400" />
                                    Last activity {toRelativeTime(template.updatedAt)}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-6 text-sm theme-text-secondary md:w-65 md:shrink-0">
                            <p className="inline-flex items-center gap-1.5">
                                ...
                                Slides <span className="font-semibold theme-text-primary">{template.questions?.length || 0}</span>
                            </p>
                            <p className="inline-flex items-center gap-1.5">
                                <Radio size={13} className="text-gray-400" />
                                Sessions Live <span className="font-semibold theme-text-primary">{template.sessionCount ?? 0}</span>
                            </p>
                        </div>

                        <div className="flex flex-wrap items-center gap-2 md:flex-1">
                            <span className={getVisibilityTag(template.accessType)}>{template.accessType === 'private' ? 'Private' : 'Public'}</span>
                            <span className={getModeTag(template.mode)}>{template.mode === 'tutor' ? 'Tutor' : 'AutoTime'}</span>
                            {String(template.status || '').toLowerCase() === 'waiting' && (
                                <span className={`${tagStyles.base} ${tagStyles.upcoming}`}>Waiting</span>
                            )}
                            {String(template.status || '').toLowerCase() === 'live' && (
                            ...
                            <div className="relative" ref={menuRef}>
                                ...
                                {showMenu && (
                                    <div className={cardStyles.menuPanel} onClick={(event) => event.stopPropagation()}>
                                        <button type="button" onClick={() => handleMenuAction(onSessionSettings)} className={`${cardStyles.menuItem} flex items-center gap-2 text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20`}>
                                            <Settings2 size={13} />
                                            Session Settings
                                        </button>
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => handleMenuAction(() => onDelete(template._id))}
            ...
            >
                <div className={isList ? cardStyles.contentGrid : cardStyles.contentGridDefault}>
                    <div className={cardStyles.headerRow}>
                        <h3 className={`${textStyles.title} truncate`} title={template.title}>
                            {template.title}
                        </h3>
                        <div className="relative shrink-0" ref={menuRef}>
                            ...
                            {showMenu && (
                                ...
                                >
                                    <button
                                        type="button"
                                        onClick={() => handleMenuAction(onSessionSettings)}
                                        className={`${cardStyles.menuItem} flex items-center gap-2 text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20`}
                                    >
                                        <Settings2 size={13} />
                                        Session Settings
                                    </button>
