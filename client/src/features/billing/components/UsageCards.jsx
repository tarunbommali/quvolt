...
const UsageCards = ({ usage, limitFree, commLimit, participantLimit }) => {
    ...
    const progressPercent = isUnlimited ? 100 : Math.min((usage.freeCreated / limitFree) * 100, 100);
    ...
    <div className={cardStyles.usageGrid}>
        <Card className={`${cardStyles.usageCard} group hover:border-indigo-200`}>
            ...
            <div className={`${cardStyles.valueRow} relative`}>
                <span className={`text-3xl font-semibold ${isMaxedOut ? 'text-red-500' : 'text-slate-900'}`}>
                    {usage.freeCreated}
                </span>
                <span className="mb-1 text-sm text-slate-400">/ {limitFree} {isUnlimited ? '' : 'Allowed'}</span>
            </div>
            <div className={cardStyles.usageProgressWrap}>
                <div
                    className={`${cardStyles.usageProgressFill} ${isMaxedOut ? 'bg-red-500' : 'bg-indigo-500'}`}
