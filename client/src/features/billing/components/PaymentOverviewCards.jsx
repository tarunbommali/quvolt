        ...
        <div className="relative overflow-hidden rounded-[1.75rem] border theme-border theme-surface p-6 shadow-sm theme-interactive">
            ...
            <div className="relative space-y-5">
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between rounded-xl border theme-border theme-surface-soft px-4 py-3">
                        ...
                        <span
                            className={cx(
                                'text-sm font-bold capitalize',
                                hostAccount?.accountStatus === 'verified' ? 'theme-tone-success' : 'theme-tone-warning',
                            )}
                        >
                ...
                <div className="space-y-3">
                    <div className={cx("rounded-xl border px-4 py-3 text-xs font-bold", 
                        hostAccount?.accountStatus === 'verified' ? 'theme-status-success' : 'theme-status-warning'
                    )}>
                    ...
                    {hostAccount?.accountStatus !== 'verified' && (
                        <button
                            ...
                            className="w-full flex justify-center items-center rounded-xl bg-[var(--qb-primary)] px-4 py-2.5 text-sm font-semibold text-white transition-all hover:bg-[color-mix(in_srgb,var(--qb-primary)_85%,black)] disabled:opacity-50"
                        >
                            {hostAccount?.isLoadingKyc ? 'Processing...' : hostAccount?.accountStatus === 'not_started' || !hostAccount ? 'Create Connected Account' : 'Complete KYC'}
                        </button>
                    )}
                </div>
            </div>
