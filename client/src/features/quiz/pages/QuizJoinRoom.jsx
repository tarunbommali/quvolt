...
import { Hash, AlertCircle, Loader2, Sparkles, User, ArrowRight } from 'lucide-react';
...
import LanguagePickerScreen from '../../../components/i18n/LanguagePickerScreen';
...
const QuizJoinRoom = () => {
    ...
    const handleJoin = async (e) => {
        ...
        try {
            ...
            // [I18N] Check if language selection is required
            if (quiz.requireLanguageSelection && quiz.availableLanguages?.length > 1) {
                setShowLanguagePicker(true);
                setLoading(false);
                return;
            }

            proceedToJoin(cleanedCode, null);
        } catch (err) {
    ...
    const proceedToJoin = (code, lang) => {
        if (!user) {
            setShowGuestForm(true);
            return;
        }
        navigate(`/quiz/${code}`, { state: { preferredLanguage: lang } });
    };

    const handleLanguageSelected = (lang) => {
        setPreferredLanguage(lang);
        setShowLanguagePicker(false);
        proceedToJoin(sanitizeRoomCode(roomCode), lang);
    };
    ...
    const handleGuestJoin = async (e) => {
        ...
        try {
            ...
            const cleanedCode = sanitizeRoomCode(roomCode);
            await getQuizByCodeCached(cleanedCode);
            navigate(`/quiz/${cleanedCode}`, { state: { preferredLanguage } });
        } catch (err) {
        ...
        <div className="min-h-screen flex items-center justify-center p-6 bg-gradient-to-br from-indigo-500/5 to-purple-500/5">
            ...
            >
                ...
                <div className={`${components.analytics.card} !p-12 !rounded-[3.5rem] relative overflow-hidden border-2 border-white/50 dark:border-white/5 backdrop-blur-xl shadow-2xl shadow-indigo-500/10`}>
                    ...
                    {showLanguagePicker && fetchedQuiz && (
