...
const useQuizSession = ({ sessionCode, sessionId, role = 'participant' } = {}) => {
    ...
    // joinRoom is idempotent (see useSocketStore) — safe to call on every
    // reconnect without double-joining, since it guards against duplicate rooms.
    useEffect(() => {
        if (!sessionCode) return;
        if (!socket || !connected) return;
        joinRoom(sessionCode.toUpperCase(), sessionId);
    }, [sessionCode, sessionId, connected]); // eslint-disable-line react-hooks/exhaustive-deps
